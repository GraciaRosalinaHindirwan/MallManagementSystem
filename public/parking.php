...
function addTarif(
    ...
    if (!$useDb || !$pdo) return ['success'=>false,'message'=>'Database tidak tersedia.'];

    try {
        $pdo->prepare(
            "INSERT INTO 04_parking_tarif
               (tipe_kendaraan, tipe_user, tarif_jam_pertama, tarif_per_jam, tarif_harian_max, berlaku_dari, berlaku_sampai)
             VALUES (?,?,?,?,?,?,?)"
        )->execute([$tipeKendaraan,$tipeUser,$tarifJ1,$tarifPjm,$tarifMaks,$berlakuDari,$berlakuSampai]);
        return ['success'=>true,'message'=>"Tarif $tipeKendaraan/$tipeUser berhasil ditambahkan.",'state'=>getParkingState()];
    } catch (Throwable $e) {
        error_log('[addTarif] '.$e->getMessage());
        return ['success'=>false,'message'=>'Gagal tambah tarif: '.$e->getMessage()];
    }
}

function deleteTarif(int $idTarif): array
{
    global $useDb, $pdo;
    if (!$useDb || !$pdo) return ['success'=>false,'message'=>'Database tidak tersedia.'];
    try {
        $stmt = $pdo->prepare('DELETE FROM 04_parking_tarif WHERE id_tarif=?');
        $stmt->execute([$idTarif]);
        if ($stmt->rowCount()===0) return ['success'=>false,'message'=>'Tarif tidak ditemukan.'];
        return ['success'=>true,'message'=>'Tarif berhasil dihapus.','state'=>getParkingState()];
    } catch (Throwable $e) {
        error_log('[deleteTarif] '.$e->getMessage());
        return ['success'=>false,'message'=>'Gagal hapus tarif: '.$e->getMessage()];
    }
}
