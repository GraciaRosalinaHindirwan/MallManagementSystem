...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        $stmt = $pdo->prepare("INSERT INTO pegawai (nik,nama,jabatan,departemen,email,no_hp,alamat,tgl_lahir,tgl_masuk) VALUES (?,?,?,?,?,?,?,?,?)");
        $stmt->execute([$nik,$nama,$jabatan,$departemen,$email,$no_hp,$alamat,$tgl_lahir ?: null,$tgl_masuk]);
    ...
    <form method="POST">
        <div class="form-grid">
            <div class="form-group">
                ...
                <input type="text" name="nik" value="<?= htmlspecialchars($_POST['nik'] ?? '') ?>" placeholder="EMP001" required>
            </div>
            <div class="form-group">
                <label>Nama Lengkap <span style="color:var(--danger)">*</span></label>
                <input type="text" name="nama" value="<?= htmlspecialchars($_POST['nama'] ?? '') ?>" placeholder="Nama pegawai" required>
            </div>
            <div class="form-group">
                <label>Jabatan <span style="color:var(--danger)">*</span></label>
                <input type="text" name="jabatan" value="<?= htmlspecialchars($_POST['jabatan'] ?? '') ?>" placeholder="Staff HR, Kasir, dll" required>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input type="email" name="email" value="<?= htmlspecialchars($_POST['email'] ?? '') ?>" placeholder="email@mall.com" required>
            </div>
            <div class="form-group">
                <label>No. HP</label>
            ...
            <div class="form-group">
                ...
                <input type="date" name="tgl_lahir" value="<?= $_POST['tgl_lahir'] ?? '' ?>">
            </div>
            <div class="form-group">
                <label>Tanggal Masuk <span style="color:var(--danger)">*</span></label>
                <input type="date" name="tgl_masuk" value="<?= $_POST['tgl_masuk'] ?? date('Y-m-d') ?>" required>
            </div>
            <div class="form-group" style="grid-column: 1/-1;">
                <label>Alamat</label>
                <textarea name="alamat" rows="3" placeholder="Alamat lengkap"><?= htmlspecialchars($_POST['alamat'] ?? '') ?></textarea>
            </div>
        </div>
