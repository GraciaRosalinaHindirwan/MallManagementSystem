...
// ============================================================
// 3. QUERY UTAMA: Ambil hanya 1 work order terbaru per report
// ============================================================
$tickets = mysqli_query($conn, "
    ...
    LEFT JOIN (
        -- SUBQUERY: ambil work_order terbaru per report_id (berdasarkan work_order_id DESC)
        SELECT wo1.*
...
// ============================================================
// 4. QUERY DETAIL DRAWER (juga ambil 1 work order terbaru)
// ============================================================
$ticketDetails = [];
...
<!-- CSS internal untuk drawer (sama seperti sebelumnya) -->
<style>
    #drawerOverlay {
        position: fixed;
        inset: 0;
    ...
    .bg-accent\/10 { background: rgba(0,212,216,0.1); }
    .bg-success\/10 { background: rgba(34,197,94,0.1); }
    .bg-secondary\/10 { background: rgba(22,126,128,0.1); }
    .bg-secondary { background: var(--secondary, #167E80); }
    .bg-accent { background: var(--accent, #00D4D8); }
    .text-primary-dark { color: var(--primary-dark, #082A53); }
    .text-on-surface-variant { color: rgba(245,247,250,0.6); }
    .text-accent { color: var(--accent, #00D4D8); }
...
<aside id="detailDrawer">
    ...
    <div class="drawer-body">
        ...
        <div style="display:grid; grid-template-columns:1fr; gap:12px; margin-top:8px;">
            ...
            <a id="closeBtn" href="#" class="btn" style="border:1px solid #EF4444; color:#EF4444; justify-content:center; width:100%; background:transparent;">Close Ticket</a>
        </div>
    </div>
</aside>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        window.openDrawer = function(reportId) {
            ...
            document.getElementById('drawerStatus').innerText = data.status || 'N/A';
            ...
            document.getElementById('closeBtn').href = 'Damage_List.php?close=' + data.report_id;
            ...
            timeline.forEach(item => {
                html += `
                    <div class="card">
                        <div style="font-weight:600;">${item.activity_type || ''}</div>
                        <div style="font-size:12px; color:rgba(245,247,250,0.6);">${item.activity_note || ''}</div>
                        <div style="font-size:12px; color:var(--accent, #00D4D8); margin-top:4px;">${item.created_at || ''}</div>
                    </div>
                `;
            });
