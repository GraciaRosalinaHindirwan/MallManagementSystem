...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    checkSubmission($submissionId, $tenantId, 'tagihan.php');
    ...
    if ($action === 'pay_invoice') {
        ...
        try {
            ...
            redirectToPage('tagihan.php', $tenantId, 'Pembayaran berhasil diproses. Status invoice menjadi Lunas.', 'success');
        } catch (PDOException $e) {
            redirectToPage('tagihan.php', $tenantId, 'Pembayaran gagal: ' . $e->getMessage(), 'error');
        }
    }

    redirectToPage('tagihan.php', $tenantId);
}
?>
<!DOCTYPE html>
<html lang="id">
...
<div class="layout">
    <aside class="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a class="nav-item active" href="tagihan.php?tenant_id=<?= (int) $tenantId ?>">
                <span>💳</span>
                <span>Tagihan</span>
            </a>
        ...
        <section class="content-body">
            <div class="page-wrapper">
                ...
                <div class="page-header">
                    <div>
                        <p class="page-breadcrumb">Tenant Portal / Tagihan</p>
                        <h1 class="page-title">Tagihan & Pembayaran</h1>
                        <p class="page-description">
                            Tenant dapat melihat daftar tagihan dan melakukan pembayaran.
                        </p>
                    </div>
                </div>
                ...
                <div class="card">
                    <div class="info-box">
                        Ketika tombol bayar ditekan, status invoice akan berubah menjadi <strong>Lunas</strong> di database.
                    </div>
                    ...
                    <div class="table-wrapper">
                        <table class="data-table">
                            ...
                            <tbody>
                                <?php if (empty($bills)): ?>
                                    <tr>
                                        <td colspan="6" class="empty-row">Belum ada invoice.</td>
                                    </tr>
                                <?php else: ?>
                                    <?php foreach ($bills as $index => $bill): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <?php if ($bill['status'] === 'Lunas'): ?>
                                                    <button class="btn-secondary" type="button" disabled>Selesai</button>
                                                <?php else: ?>
                                                    <form method="POST" action="tagihan.php?tenant_id=<?= (int) $tenantId ?>" style="display:inline;">
                                                        <input type="hidden" name="action" value="pay_invoice">
                                                        <input type="hidden" name="submission_id" value="<?= htmlspecialchars($paymentSubmissionBase . '_' . (int) $bill['id'] . '_' . $index) ?>">
                                                        <input type="hidden" name="invoice_id" value="<?= (int) $bill['id'] ?>">
                                                        <input type="hidden" name="payment_method" value="Bank Transfer">
                                                        <button class="btn-primary" type="submit" onclick="return confirm('Proses pembayaran invoice ini?')">
                                                            Bayar
                                                        </button>
                                                    </form>
                                                <?php endif; ?>
